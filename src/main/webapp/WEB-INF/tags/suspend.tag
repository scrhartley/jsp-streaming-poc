...
<c:set var="__suspendIdCounter__" scope="request" value="${requestScope.__suspendIdCounter__ + 1}" />
<c:set var="id" value="JSUSP_fallback:${requestScope.__suspendIdCounter__}" />
<c:choose>
    ...
    <c:otherwise><template id="${id}">${requestScope.sharedSuspendFallback}</template></c:otherwise>
</c:choose>
